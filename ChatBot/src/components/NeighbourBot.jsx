...
const NeighbourBot = ({ jwtToken, apiBase, neighbourhoodId, onNeighbourhoodUpdate, backendUrl, useBackend = true }) => {
  ...
  useEffect(() => {
    ...
    const apiBaseUrl = apiBase || config.chatbot_config.authentication.api_base;
    ...
    try {
      ...
      if (!result.intent) {
        ...
        if (groqError) {
          ...
          // Optionally show a subtle error message, but continue with fallback
          // fallbackMsg = `${groqError.errorMessage} ${fallbackMsg}`;
        }
